...
// فونت وزیر باید به Base64 تبدیل و اینجا import شود
import vazirFont from "@/public/fonts/Vazir-Regular-normal";
...
export function generateAndPrintReceipt(data: ReceiptData) {
  ...
  const rightMargin = 550; // برای راست‌چین
  let y = 40;

  const toPersianNumber = (n: number | string) =>
    n?.toString().replace(/[0-9]/g, (d) => "۰۱۲۳۴۵۶۷۸۹"[parseInt(d)]) || "۰";

  const drawText = (txt: string, offsetY: number, fontSize = 12) => {
    doc.setFontSize(fontSize);
    doc.text(txt || "-", rightMargin, y + offsetY, { align: "right" });
    y += offsetY;
  };
  ...
  const serviceRows = services.length
    ? services.map((s) => [s.name || "-", toPersianNumber(s.price || 0)])
  ...
  drawText(`جمع کل: ${toPersianNumber(data.total_payment || 0)} تومان`, 15, 14);
  drawText(`بیمه پایه: ${toPersianNumber(data.insurance_base || 0)} تومان`, 12);
  drawText(`بیمه تکمیلی: ${toPersianNumber(data.insurance_extra || 0)} تومان`, 12);
