...
export default function DoctorsPage() {
  ...
  const fetchAvailableTimes = async (doctorId: string, day: string) => {
    try {
      ...
      const res = await api.post<{ bookedTimes: string[] }>("api/appointment/available", { doctorId, appointmentDate });
  ...
  const handleReserve = async () => {
    if (!selectedDoctor || !selectedDay || !selectedTime) { toast.error("لطفاً روز و ساعت را انتخاب کنید"); return; }
    ...
    if (!formData.fullName || !/^09\d{9}$/.test(phone) || national.length !== 10 || !formData.insuranceType) {
      toast.error("اطلاعات وارد شده معتبر نیست"); return;
    }
    ...
    try {
      ...
      setSelectedDoctor(null); setStep(1);
  ...
  if (loading) return <p className="text-center mt-10 text-gray-300">در حال بارگذاری پزشکان...</p>;
      ...
      <h1 className="text-3xl font-bold mb-8 text-center text-white">
        {pathname.includes("general") ? "پزشکان عمومی" :
          pathname.includes("dentist") ? "پزشکان دندان پزشک" :
            pathname.includes("specialist") ? "پزشکان متخصص" : "لیست پزشکان"}
      </h1>
      ...
      {doctors.length === 0 ? (
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {doctors.map((doctor) => {
            ...
            return (
              <div key={doctor.personnelId} className="bg-white/20 backdrop-blur-md border border-white/30 rounded-3xl shadow-lg hover:shadow-xl transition p-6 flex flex-col items-center">
                <Image src={doctor.avatarUrl || "/images/defult.png"} alt={doctor.doctorName} width={112} height={112} className="rounded-full object-cover mb-4 border-2 h-full" />
                <h3 className="text-xl font-semibold mb-1 text-center text-white">{doctor.doctorName}</h3>
                ...
                <button
                  ...
                  disabled={isDentist}
                  className={`w-full py-2 rounded-xl transition font-medium ${isDentist ? "bg-gray-500/50 text-gray-300 cursor-not-allowed" : "bg-blue-600/70 text-white hover:bg-blue-700/80"}`}
                >
        ...
        <Modal onClose={() => setSelectedDoctor(null)}>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {Object.keys(selectedDoctor.workingHours).length > 0 ? (
                  Object.keys(selectedDoctor.workingHours).map((day) => (
                    <button key={day} onClick={() => { setSelectedDay(day); setStep(2); setSelectedTime(null); fetchAvailableTimes(selectedDoctor.personnelId, day); }} className={`px-3 py-1 rounded ${selectedDay === day ? "bg-blue-500 text-white" : "bg-gray-200/50"}`}>{day}</button>
                  ))
          ...
          {step === 2 && selectedDay && (
            <>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {selectedDoctor.workingHours[selectedDay]?.shifts?.flatMap(s => generateTimeBlocks(s.start, s.end)).map(time => {
                  const isBooked = availableTimes.includes(time);
                  return <button key={time} disabled={isBooked} onClick={() => { setSelectedTime(time); setStep(3); }} className={`px-3 py-1 rounded transition ${isBooked ? "bg-red-400 text-white cursor-not-allowed" : selectedTime === time ? "bg-green-500 text-white" : "bg-gray-200/50"}`}>{time}</button>;
